...
def create_graph():
    ...
    def do_load_price_history(symbol, interval):
        url = 'https://www.binance.com/api/v3/klines?symbol=%s&interval=%s&limit=%s' % (symbol, interval, 1000)
    ...
    def calc_plot_data(df, indicators):
        ...
        ma2 = ma5 = vema24 = sar = rsi = stoch = stoch_s = None
        if 'few' in indicators or 'moar' in indicators:
            ma2  = price.Close.rolling(2).mean()
        ...
        plot_data = dict(price=price, volume=volume, ma2=ma2, ma5=ma5, vema24=vema24, sar=sar, rsi=rsi, \
    ...
    def change_asset(*args, **kwargs):
        ...
        if data['ma5'] is not None:
            plots['ma2'] = fplt.plot(data['ma2'], legend='MA-2', ax=ax)
    ...
    def create_ctrl_panel(win):
        ...
        [panel.symbol.addItem(i+'USDT') for i in 'BTC EUR ETH XRP DOGE BNB SOL ADA LTC LINK DOT TRX BCH'.split()]
        ...
        [panel.indicators.addItem(i) for i in 'Clean:Few indicators:More indicators'.split(':')]
